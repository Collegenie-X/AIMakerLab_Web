...
export function CurriculumSection({
  ...
  return (
    <CurriculumSectionContainer className="py-12 bg-gray-950" containerClass={containerClass}>
      <h2 className="mb-8 text-3xl font-bold text-center text-white">{title}</h2>
      ...
      <div className="mb-8 text-center">
        <div className={`inline-flex items-center gap-2 bg-${primaryColor}-900/20 px-4 py-2 rounded-full mb-3`}>
          <Clock className={`h-4 w-4 text-${primaryColor}-600`} />
          <span className={`text-sm font-medium text-${primaryColor}-400`}>
          ...
          <Card key={module.id} className="bg-gray-900 border-gray-700">
            <CardContent className="pt-6">
              ...
              {module.detailDescription && (
                <p className="text-sm text-gray-400 mb-3 pl-16">{module.detailDescription}</p>
              )}
      ...
      {/* 총 시간 요약 */}
      <div className={`mt-8 p-6 bg-gradient-to-r from-${primaryColor}-900/20 to-purple-900/20 rounded-lg border border-${primaryColor}-800`}>
        <div className="flex items-center justify-center gap-2 text-center">
          <div className={`flex items-center gap-2 text-${primaryColor}-400`}>
            <Clock className="h-5 w-5" />
            <span className="font-semibold">총 학습 시간:</span>
            <span className={`text-xl font-bold text-${primaryColor}-500`}>{activeTab.duration}</span>
          </div>
        </div>
      </div>
