...
```mermaid
graph LR
    subgraph "1단계: 피지컬 컴퓨팅 기초"
        A1[센서 <br>입력]
        A2[프로그래밍]
        A3[액추에이터<br> 출력]
    end
    
    subgraph "2단계:원격 제어 & 데이터"
        B1[WiFi/<br>서버 연결]
        B2[원격 제어]
        B3[데이터 <br>수집·저장]
    end
...
```mermaid
timeline
    title 피지컬 컴퓨팅에서 피지컬 AI로
    
    2000-2010 : 1세대 피지컬 컴퓨팅
              : Arduino 등장
              : LED, 센서 기초 제어
              : 규칙 기반 프로그래밍
    
    2010-2020 : 2세대 IoT 연결
              : ESP32, WiFi 모듈
              : 원격 제어, 클라우드 연동
              : 데이터 수집 및 분석
    
    2020-2026 : 3세대 피지컬 AI
              : Raspberry Pi + AI
              : 컴퓨터 비전, 딥러닝
              : 자율 판단 및 행동
              : CES 2026 메인 테마
...
```mermaid
graph TB
    subgraph "1단계: 피지컬 컴퓨팅 기초 (Arduino)"
        S1A[디지털 입출력<br/>LED, 버튼]
        S1B[아날로그 센서<br/>온도, 조도]
        S1C[액추에이터<br/>서보, DC 모터]
        S1D[알고리즘<br/>조건문, 반복문]
        
        S1A --> S1B --> S1C --> S1D
    end
    
    subgraph "2단계: 원격 제어 & 데이터 (ESP32)"
        S2A[WiFi 연결<br/>서버 통신]
        S2B[웹 인터페이스<br/>원격 제어]
        S2C[데이터 수집<br/>센서 로깅]
        S2D[클라우드 연동<br/>실시간 모니터링]
        
        S2A --> S2B --> S2C --> S2D
    end
    
    subgraph "3단계: 피지컬 AI (Raspberry Pi)"
        S3A[컴퓨터 비전<br/>OpenCV]
        S3B[객체 인식<br/>YOLO]
        S3C[딥러닝 추론<br/>TensorFlow Lite]
        S3D[자율 로봇<br/>AI 판단 + 행동]
    ...
    style S1A fill:#10b981,color:#fff
    style S2A fill:#3b82f6,color:#fff
    style S3A fill:#8b5cf6,color:#fff
    style S3D fill:#ef4444,color:#fff
```
...
graph LR
    subgraph "1단계: 로컬"
        A1[센서] --> A2[Arduino] --> A3[액추에이터]
        A4[사람이 직접 있어야 함]
    end
    
    subgraph "2단계: 원격"
        B1[센서] --> B2[ESP32] --> B3[WiFi]
        B3 --> B4[클라우드]
        B4 --> B5[스마트폰/PC]
        B5 --> B3
        B3 --> B6[액추에이터]
    ...
    subgraph "3단계로 연결"
        C1[수집된 데이터] --> C2[AI 학습 데이터셋]
    end
    
    A3 -.한계.-> B1
    B4 --> C1
    
    style A2 fill:#94a3b8,color:#fff
    style B2 fill:#3b82f6,color:#fff
    style C2 fill:#8b5cf6,color:#fff
```
...
```mermaid
graph TB
    subgraph "AI 두뇌: Raspberry Pi 또는 PC"
        A1[카메라 입력]
        A2[컴퓨터 비전<br/>OpenCV]
        A3[딥러닝 추론<br/>YOLO]
        A4[AI 판단<br/>의사결정]
    ...
    subgraph "통신 계층"
        B1[Serial<br/>USB 케이블]
        B2[WiFi<br/>무선 통신]
        B3[MQTT<br/>메시지 프로토콜]
    end
    
    subgraph "제어 시스템: Arduino 또는 ESP32"
        C1[센서 읽기<br/>초음파, 온도 등]
        C2[모터 제어<br/>DC, 서보]
        C3[액추에이터<br/>LED, 릴레이]
...
```mermaid
graph LR
    A[카메라 이미지] --> B[YOLO 신경망]
    B --> C1[사람 80%]
    B --> C2[자동차 75%]
    B --> C3[고양이 92%]
    
    C1 --> D[바운딩 박스<br/>좌표 + 라벨]
    C2 --> D
    C3 --> D
...
```mermaid
graph TB
    subgraph "AI 시스템: Raspberry Pi"
        A1[카메라] --> A2[YOLO 물체 인식]
        A2 --> A3{물체 종류?}
        
        A3 -->|플라스틱| A4[명령: PLASTIC]
        A3 -->|종이| A5[명령: PAPER]
        A3 -->|캔| A6[명령: CAN]
    end
    
    subgraph "제어 시스템: Arduino"
        B1[Serial 수신]
        B2[서보 모터 1<br/>분류 게이트]
        B3[서보 모터 2<br/>푸셔]
        B4[LED 표시]
